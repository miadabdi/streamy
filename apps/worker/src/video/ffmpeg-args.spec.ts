...
describe('ffmpeg argument builders', () => {
	...
	it('omits -start_number by default and inserts it in the hls block when given', () => {
		const fresh = buildLiveArgs('rtmp://x', 8, SOFTWARE_PLAN);
		expect(fresh).not.toContain('-start_number');

		const resumed = buildLiveArgs('rtmp://x', 8, SOFTWARE_PLAN, 7);
		expect(resumed[resumed.indexOf('-start_number') + 1]).toBe('7');
		expect(resumed.indexOf('-start_number')).toBeGreaterThan(resumed.indexOf('-f'));
		expect(resumed.indexOf('-start_number')).toBeLessThan(resumed.indexOf('-hls_segment_filename'));
		// explicit 1 stays the default behavior: no flag
		expect(buildLiveArgs('rtmp://x', 8, SOFTWARE_PLAN, 1)).not.toContain('-start_number');
	});
